...
class Boot {
  def boot {
    if (!DB.jndiJdbcConnAvailable_?) {
      val vendor =
        new StandardDBVendor(Props.get("mysql.db.driver") openOr "org.h2.Driver",
          Props.get("mysql.db.url") openOr  "jdbc:h2:lift_proto.db;AUTO_SERVER=TRUE",
          Props.get("mysql.db.user"), Props.get("mysql.db.pass"))
    ...
    // any ORM you want
    Schemifier.schemify(true, Schemifier.infoF _, User, Album)
